...
if __name__ == "__main__":
    ...
    snDirCount = os.listdir(segfilepath)
    for curDir in snDirCount:
        snDirPath   = os.path.join(segfilepath, curDir)
        psnCount    = os.listdir(snDirPath)
        for curPsn in psnCount:
            psnPath     = os.path.join(snDirPath, curPsn)
            typeCount   = os.listdir(psnPath)
            for curType in typeCount:
                imagePath    = os.path.join(psnPath, curType)

                temp_seg = os.listdir(imagePath)
                total_seg = []
                for seg in temp_seg:
                    if seg.endswith(".png"):
                ...
                for i in list:  
                    name = f"{curDir}/{curPsn}/{curType}/" + total_seg[i][:-4]+'\n'  
                    if i in trainval:  
                ...
                for i in tqdm(list):
                    name            = total_seg[i]
                    png_file_name   = os.path.join(segfilepath, curDir, curPsn, curType, name)
